<template>
  <section id="schedule" aria-label="Schedule" class="py-20 sm:py-32">
    ...
    <div class="relative mt-14 sm:mt-24">
      <BackgroundImage position="right" class="-top-40 -bottom-32" />
      <Container class="relative">
        ...
        >
          <TabList class="-mx-4 flex gap-x-4 gap-y-10 overflow-x-auto pb-4 pl-4 sm:mx-0 sm:flex-col sm:pr-8 sm:pb-0 sm:pl-0">
            <div
              v-for="(day, dayIndex) in schedule"
              :key="day.dateTime"
              :class="['relative w-3/4 flex-none pr-4 sm:w-auto sm:pr-0', dayIndex !== selectedIndex && 'opacity-70']"
            >
              <h3 class="text-2xl font-semibold tracking-tight text-blue-900">
                <Tab as="template">
                  <button class="relative text-left focus:outline-none">
          ...
          <TabPanels>
            ...
            >
              <ol
                role="list"
                class="space-y-8 bg-white/60 px-10 py-14 text-center shadow-xl shadow-blue-900/5 backdrop-blur-sm"
              >
                <li
                  v-for="(timeSlot, timeSlotIndex) in day.timeSlots"
                  :key="timeSlot.start"
        ...
        <div class="hidden lg:grid lg:grid-cols-3 lg:gap-x-8">
          <section v-for="day in schedule" :key="day.dateTime">
            ...
            <ol
              role="list"
              class="mt-10 space-y-8 bg-white/60 px-10 py-14 text-center shadow-xl shadow-blue-900/5 backdrop-blur-sm"
            >
              <li
                v-for="(timeSlot, timeSlotIndex) in day.timeSlots"
                :key="timeSlot.start"
