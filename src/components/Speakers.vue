<template>
  <section id="speakers" aria-labelledby="speakers-title" class="py-20 sm:py-32">
    <svg aria-hidden="true" width="0" height="0">
      <defs>
        <clipPath :id="`${clipPathId}-0`" clipPathUnits="objectBoundingBox">
          <path d="M0,0 h0.729 v0.129 h0.121 l-0.016,0.032 C0.815,0.198,0.843,0.243,0.885,0.243 H1 v0.757 H0.271 v-0.086 l-0.121,0.057 v-0.214 c0,-0.032,-0.026,-0.057,-0.057,-0.057 H0 V0" />
        </clipPath>
        <clipPath :id="`${clipPathId}-1`" clipPathUnits="objectBoundingBox">
          <path d="M1,1 H0.271 v-0.129 H0.15 l0.016,-0.032 C0.185,0.802,0.157,0.757,0.115,0.757 H0 V0 h0.729 v0.086 l0.121,-0.057 v0.214 c0,0.032,0.026,0.057,0.057,0.057 h0.093 v0.7" />
        </clipPath>
        <clipPath :id="`${clipPathId}-2`" clipPathUnits="objectBoundingBox">
    ...
    <Container>
      ...
      >
        <div class="relative -mx-4 flex overflow-x-auto pb-4 sm:mx-0 sm:block sm:overflow-visible sm:pb-0">
          <div class="absolute top-2 bottom-0 left-0.5 hidden w-px bg-slate-200 lg:block" />
          <TabList
            class="grid auto-cols-auto grid-flow-col justify-start gap-x-8 gap-y-10 px-4 whitespace-nowrap sm:mx-auto sm:max-w-2xl sm:grid-cols-3 sm:px-0 sm:text-center lg:grid-flow-row lg:grid-cols-1 lg:text-left"
          >
            <div v-for="(day, dayIndex) in days" :key="day.dateTime" class="relative lg:pl-8">
              <DiamondIcon
                :class="[
              ...
              />
              <div class="relative">
                <div :class="['font-mono text-sm', dayIndex === selectedIndex ? 'text-blue-600' : 'text-slate-500']">
        ...
        <TabPanels class="lg:col-span-3">
          <TabPanel
            v-for="day in days"
            :key="day.dateTime"
            class="grid grid-cols-1 gap-x-8 gap-y-10 sm:grid-cols-2 sm:gap-y-16 md:grid-cols-3"
            :unmount="false"
          >
            <div v-for="(speaker, speakerIndex) in day.speakers" :key="speaker.name">
              <div class="group relative h-70 transform overflow-hidden rounded-4xl">
                <div
                  :class="[
                ...
                <div
                  ...
                  :style="{ clipPath: `url(#${clipPathId}-${speakerIndex % 3})` }"
                >
                  <img
              ...
              <p class="mt-1 text-base tracking-tight text-slate-500">{{ speaker.role }}</p>
            </div>
          </TabPanel>
        </TabPanels>
      </TabGroup>
...
const clipPathId = `speakers-${Math.random().toString(36).slice(2, 9)}`
